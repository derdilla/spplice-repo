...
try {
  ...
  // Returns true if the components of the two vectors are similar, false otherwise
  function Vector::near (other) {
    return fabs(this.x - other.x) < 0.00001 && fabs(this.y - other.y) < 0.00001 && fabs(this.z - other.z) < 0.00001;
  }
...
::ppmod.onkill <- function (ent, scr) {
  ...
  if (!("__destructors" in scope)) {
    ...
    scope.InputKill <- function ():(scope) {
      ...
      // Call destructors of all children, too
      local child = null;
      while (child = ppmod.getchild(self, child)) {
        local cscope = child.GetScriptScope();
        if (cscope && "InputKill" in cscope) cscope.InputKill();
      }
      return true;
    };
...
::ppmod.player <- class {
  ...
  constructor (player) {
    ...
    local eyename = "pplayer_eyes_" + Time() + "_" + RandomInt(0, 255);
...
::ppmod.button <- function (type, pos, ang = Vector()) {
  ...
  return ppromise(function (resolve, reject):(type, pos, ang, model) {
    ...
    ppmod.create(model).then(function (ent):(type, pos, ang, resolve) {
      ...
      // Get vector to top of button for later use
      local uvec = ent.GetUpVector();
      ...
      if (type == "prop_button" || type == "prop_under_button") {
        ...
        ppmod.brush(pos + uvec * 40, Vector(8, 8, 8), "func_rot_button", ang, true).then(function (button):(type, ent, resolve) {
      ...
      // Handle floor buttons
        ...
        if (type == "prop_under_floor_button") {
          trigger = ppmod.trigger(pos + uvec * 8.5, Vector(30, 30, 8.5), "trigger_multiple", ang);
        } else {
          trigger = ppmod.trigger(pos + uvec * 7, Vector(20, 20, 7), "trigger_multiple", ang);
        }
