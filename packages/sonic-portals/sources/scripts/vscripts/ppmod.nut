...
local ppromise_methods = {
		...
		then = function(onthen, oncatch = function(x) {
			throw x
		}) {
	...
	try {
		...
		switch (entclasses[i]) {
			...
			case CBasePlayer:
				...
				break;
			case CEnvEntityMaker:
				classname = "CEnvEntityMaker";
				break;
			case CLinkedPortalDoor:
				classname = "CLinkedPortalDoor";
				break;
			case CPortal_Player:
				classname = "CPortal_Player";
				break;
			case CPropLinkedPortalDoor:
				classname = "CPropLinkedPortalDoor";
				break;
			case CSceneEntity:
...
::ppmod.detach <- function(scr, args, stack = null) {
	...
	::ppmod.player <- class {
		...
		static target_eyes = function() {
			...
			local scrqidx = ppmod.scrq_add(function(self): (oldname) {
				self.__KeyValueFromString("Targetname", oldname)
			}, 1);
	...
	::ppmod.create <- function(cmd, key = null) {
		...
		if (key == null) {
			...
			switch (cmd.slice(0, min(cmd.len(), 17))) {
				...
				case "ent_create_portal":
					key = "cube";
					break;
				case "ent_create_paint_":
					key = "prop_paint_bomb";
					break;
...
::ppmod.ray <- class {
	...
	::ppmod.button <- function(type, pos, ang = Vector()) {
		...
		switch (type) {
			...
			case "prop_under_button": {
				model = "props_underground/underground_testchamber_button.mdl";
				break;
			}
			case "prop_floor_button": {
				model = "props/portal_button.mdl";
				break;
			}
			case "prop_floor_cube_button": {
				model = "props/box_socket.mdl";
				break;
			}
			case "prop_floor_ball_button": {
				model = "props/ball_button.mdl";
				break;
			}
			case "prop_under_floor_button": {
				model = "props_underground/underground_floor_button.mdl";
				break;
			}
			default:
				throw "button: Invalid button type";
		}
				...
				if (type == "prop_button" || type == "prop_under_button") {
					...
					ppmod.brush(pos + (ent.GetUpVector() * 40), Vector(8, 8, 8), "func_rot_button", ang, true).then(function(button): (type, ent, resolve) {
						...
						resolve({
							...
							GetProp = function(): (ent) {
								return ent
							},
							SetDelay = function(delay): (properties) {
								properties.delay = delay
							},
							SetTimer = function(enabled): (properties) {
								properties.timer = enabled
							},
							SetPermanent = function(enabled): (properties) {
								properties.permanent = enabled
							},
							OnPressed = function(scr): (button) {
								ppmod.addscript(button, "OnPressed", scr)
							},
					...
					resolve({
						...
						GetCount = function(): (properties) {
							return properties.count
						},
						OnPressed = function(scr): (pressrl) {
							ppmod.addscript(pressrl, "OnTrigger", scr)
						},
						OnUnpressed = function(scr): (unpressrl) {
							ppmod.addscript(unpressrl, "OnTrigger", scr)
						},
