...
::pprint <- function (...) {
  if (vargc == 1) {
    ...
    switch (typeof vargv[0]) {
      case "array":
        print("[ ");
        for (local i = 0; i < vargv[0].len(); i ++) {
          if (typeof vargv[0][i] == "string") {
            print("\"");
            print(vargv[0][i]);
            print("\"");
          } else ::pprint(vargv[0][i]);
          if (i != vargv[0].len() - 1) print(", ");
        }
        print(" ]");
        break;
      case "table":
...
::ppmod.portal <- function (portal) {
  ...
  scope.ppmod_portal.GetLinkageGroupID <- function ():(new_detector) {
    return ppromise(function (resolve, reject):(new_detector) {
      ...
      local scrq_idx_params = ppmod.scrq_add(function ():(params) { return params }, 1);
