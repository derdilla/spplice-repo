if("Entities" in this) {
  ...
  if(!("rev" in this)) {
    ...
    rev.spawn <- function() {
      ...
      ppmod.keyval("arrival_elevator-elevator*", "CollisionGroup", 1);
      local reverse_maps = {};
      reverse_maps["sp_a1_intro1"] <- function() {
        rev.setup <- function() {
          ...
          ppmod.addoutput(trigger, "OnStartTouch", "AutoInstance1-tonemap_global", "SetAutoExposureMax", 2);
      ...
      reverse_maps["sp_a2_bts3"] <- function() {
        ...
        rev.level.moveplate <- function(){
          EntFire("phys_panel_1_train&*", "SetSpeedReal", 120);
          ...
          if(pos.x < 10200) plate.SetAbsOrigin(pos+Vector(4,0,0));
      ...
      reverse_maps["sp_a2_bts4"] <- function() {
        rev.setup <- function() {
          ...
          ppmod.fire(playerproxy, "ForceVMGrabController");
          EntFire("exit_airlock_door-open_door","trigger");
          ...
          rev.level.setplate();
          // Remove turret spawn RNG
          Entities.FindByName(null, "turret_conveyor_1_pre_switch_case").Destroy();
          local switcher = Entities.CreateByClassname("logic_case");
          ppmod.keyval(switcher, "Targetname", "turret_conveyor_1_pre_switch_case");
          ppmod.addoutput(switcher, "OnCase01", "turret_conveyor_1_turret_register", "FireRegisteredAsActivator2");
          // Move oracle turret to catwalk
          local oracle = Entities.FindByName(null, "conveyor_turret_template");
          oracle.SetOrigin(Vector(3200, -4926, 6700));
          oracle.SetAngles(0, 0, 0);
          ppmod.get(Vector(2353, -3151, 7331.5), "trigger_once", 32).Destroy();
        }
        ...
        rev.loop <- function() {
          ...
          if(rev.level.switched && ppmod.trigger(2153, -5514.91, 6656.03, 2231.01, -5265, 6776.56, "f1d1007c4e1ac_ppmod")){
            ...
            // Spawn oracle turret
            EntFire("conveyor_turret_template", "ForceSpawn");
          }
        ...
        rev.level.unswitch <- function() {
          ...
          local switcher = Entities.CreateByClassname("logic_case");
          ppmod.keyval(switcher, "Targetname", "turret_conveyor_1_pre_switch_case");
          ppmod.addoutput(switcher, "OnCase01", "turret_conveyor_1_turret_register", "FireRegisteredAsActivator1");
        }
      ...
      reverse_maps["sp_a2_bts5"] <- function() {
        rev.setup <- function() {
          ...
          GetPlayer().SetAngles(0, 0, 0);
          SendToConsole("sv_allow_mobile_portals 1");
          ...
          local easteregg = Entities.CreateByClassname("trigger_once");
          easteregg.SetOrigin(Vector(2848, -2016, 3584));
          easteregg.SetSize(Vector(-32, -32, -4), Vector(32, 32, 4));
          ppmod.keyval(easteregg, "Solid", 3);
          ppmod.keyval(easteregg, "CollisionGroup", 2);
          ppmod.keyval(easteregg, "SpawnFlags", 1);
          ppmod.addoutput(easteregg, "OnStartTouch", "!self", "Kill");
          ppmod.addscript(easteregg, "OnStartTouch", "rev.level.easteregg()");
          ppmod.fire(easteregg, "Enable");
        }
        ...
        rev.level.easteregg <- function() {
          ScriptSteamShowURL("https://youtu.be/OrIGoSk3fqc");
        }
      }
      ...
      reverse_maps["sp_a2_core"] <- function() {
        rev.setup <- function() {
          ...
          for(local fog = ppmod.get("env_fog_controller"); fog; fog = ppmod.get("env_fog_controller", fog)) {
            if(fog.GetName() != "@environment_glados") fog.Destroy();
          }
          EntFire("@environment_glados", "SetEndDist", 3500);
        ...
        rev.level.brushes <- function() {
          local b1 = Entities.CreateByClassname("func_brush");
          b1.SetOrigin(Vector(-1840, 224, 416));
          b1.SetSize(Vector(-224, -128, -160), Vector(224, 128, 160));
          ppmod.keyval(b1, "Solid", 3);
          local b2 = Entities.CreateByClassname("func_brush");
          b2.SetOrigin(Vector(-1672, 3, 416));
          b2.SetSize(Vector(-72, -99, -160), Vector(72, 99, 160));
          ppmod.keyval(b2, "Solid", 3);
          local b3 = Entities.CreateByClassname("func_brush");
          b3.SetOrigin(Vector(-1840, -224, 416));
          b3.SetSize(Vector(-224, -128, -160), Vector(224, 128, 160));
          ppmod.keyval(b3, "Solid", 3);
          local b4 = Entities.CreateByClassname("func_brush");
          b4.SetOrigin(Vector(-2000, -4, 416));
          b4.SetSize(Vector(-64, -100, -160), Vector(64, 100, 160));
          ppmod.keyval(b4, "Solid", 3);
        }
      ...
      reverse_maps["sp_a3_jump_intro"] <- function() {
        rev.setup <- function() {
          ...
          GetPlayer().SetAngles(0, 180, 0);
          ppmod.wait("rev.level.topcube()", 1);
        ...
        rev.level.topcube <- function() {
          local cube = ppmod.get(Vector(-96, 1154, 1182), "prop_weighted_cube", 128);
          cube.SetOrigin(Vector(-1175, 1211, 1235));
          ppmod.fire(cube, "Wake", "", 0.5);
        }
      }
      ...
      reverse_maps["sp_a4_intro"] <- function() {
        ...
        rev.setup <- function() {
          ...
          EntFire("test_chamber1_platform", "SetMaxSpeed", 100);

          rev.level.ele_up <- false;
          rev.level.ele_isup <- false;
          local uptrig = rev.trigger(Vector(850, 384, 179), Vector(17, 32, 50));
